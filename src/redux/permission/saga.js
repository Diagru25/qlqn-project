...
function* getModuleList_saga(action) {
    try {
        ...
        const moduleList = yield select(state => state.permissionReducer.moduleList);
...
function* getPermissionList_saga(action) {
    try {
        ...
        const pageSize = params.page_size ? params.page_size : permissionList.page_size;
        const pageIndex = params.page_index ? params.page_index : permissionList.page_index;
        const moduleId = params.moduleId;

        if (moduleId) {
            const res = yield permissionApi.getPermissionList(moduleId, pageIndex, pageSize);
            const { page_index, page_size, items } = res.result;
...
function* listen() {
    ...
    yield takeEvery(permissionActions.types.GET_PERMISSION_LIST, getPermissionList_saga);
    yield takeEvery(permissionActions.types.UPDATE_PERMISSION, updatePermission_saga);}
